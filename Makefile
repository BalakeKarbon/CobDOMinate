...
$(BUILD_DIR)/example/example.c: all | $(BUILD_DIR)/example 
	cobc -C -o $@ ./example/example.cob $(EXAMPLE_BASE_FLAGS) -K CHECK-COOKIE
...
$(BUILD_DIR)/example/example.js: $(BUILD_DIR)/example/example.c
	emcc -o $@ $< $(BUILD_DIR)/lib/libcobdom.a -lgmp -lcob -s EXPORTED_FUNCTIONS=_cob_init,_EXAMPLE -s EXPORTED_RUNTIME_METHODS=ccall,cwrap
	#-s STANDALONE_WASM=1
