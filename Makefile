...
$(BUILD_DIR)/example/example.c: all | $(BUILD_DIR)/example 
	cobc -C -o $@ ./example/example.cob -K cobdom_get_cookie -K cobdom_create_element -K cobdom_inner_html -K cobdom_append_child -K cobdom_remove_child -K cobdom_add_event_listener -K CHECK-COOKIE
...
$(BUILD_DIR)/example/example.js: $(BUILD_DIR)/example/example.c
	emcc -o $@ $< $(BUILD_DIR)/lib/libcobdom.a -lgmp -lcob -s EXPORTED_FUNCTIONS=_cob_init,_EXAMPLE,_CALLBACK -s EXPORTED_RUNTIME_METHODS=ccall,cwrap
	#-s STANDALONE_WASM=1
